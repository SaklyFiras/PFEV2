...
exports.getPosts = catchAsyncErrors(async (req, res, next) => {
		...
		const apiFeatures = new APIFeatures(Post.find().populate("user", "name avatar")
		.populate({
			path: "comments",
			populate: {
				path: "user",
...
exports.getUserPosts = catchAsyncErrors(async (req, res, next) => {
	const resPerPage =4;
	const postsCount = await Post.countDocuments();
	const apiFeatures = new APIFeatures(Post.find({ user: req.params.id }).populate("user", "name avatar")
	.populate({
		path: "comments",
		populate: {
			path: "user",
